...
const stages=[
 {mass:2,distance:3,right:2,title:['The first latch','第一道門閂'],mission:['The left load is fixed. Move your matching load along the right side. Move it until the beam stays level.','左邊負載已固定。把相同的負載沿右邊移動。移動它，直到橫桿保持水平。']},
...
function render(){window.OrionLayout?.update(stage, solved, 3);const c=stages[stage];document.title=t('The Counterweight Vault · Orion’s Expedition','配重密庫 · Orion 的探險');$('stageLabel').textContent=t(`SEAL ${stage+1} OF 3 · ENGINEERING`,`第 ${stage+1} 道封印，共 3 道 · 工程`);$('stageTitle').textContent=t(...c.title);$('mission').textContent=t(...c.mission);$('fixedLoad').textContent=t(`Left side: ${c.mass} identical blocks, ${c.distance} spaces from the pivot.`,`左邊：${c.mass} 個相同砝碼，距離支點 ${c.distance} 格。`);$('mass').value=String(mass);$('mass').disabled=stage<2;$('distance').value=String(distance);$('controls').hidden=solved;
 $('ticks').innerHTML=Array.from({length:13},(_,i)=>{const n=i-6,x=320+n*43;return n?`<path d="M${x} 114v14" stroke="#f4d896"/><text x="${x}" y="151" text-anchor="middle">${Math.abs(n)}</text>`:'';}).join('');$('leftLoad').innerHTML=blocks(320-c.distance*43,c.mass);$('rightLoad').innerHTML=blocks(320+distance*43,mass);$('beam').setAttribute('transform',`rotate(${(result||outcome())==='left'?-9:(result||outcome())==='right'?9:0} 320 121)`);$('pivotLabel').textContent=t('Pivot','支點');$('diagram').setAttribute('aria-label',t(`Lever: left ${c.mass} blocks at distance ${c.distance}; right ${mass} blocks at distance ${distance}.`,`槓桿：左邊 ${c.mass} 個砝碼，距離 ${c.distance}；右邊 ${mass} 個，距離 ${distance}。`));$('reading').textContent=outcomeText(result||outcome());
 const texts={invalid:t('Choose a valid number of blocks and distance.','請選擇有效的砝碼數量及距離。'),observe:t('The beam tilts. Move the weight until both sides stay level.','橫桿傾斜了。移動砝碼，直到兩邊保持水平。'),balanced:t('The beam stays level. The bronze seal releases!','橫桿保持水平，青銅封印解開了！'),another:t('One balanced design found. Change the blocks and position to find a different one.','找到一種平衡設計了。改變砝碼和位置，找出另一種。'),duplicate:t('You already recorded this design. Find a different balanced combination.','這個設計已經記錄過了。找出另一種平衡組合。'),complete:t('Two different designs, the same balance. The vault is yours to explore!','兩種不同設計，同樣達到平衡。密庫等你探索！')};$('feedback').textContent=texts[feedback]||'';
 ...
 $('log').innerHTML=history.length?history.map(h=>`<li>${t(`${h.mass} blocks at distance ${h.distance}`,`${h.mass} 個砝碼，距離 ${h.distance}`)} — ${t('observed','觀察')} ${outcomeText(h.result)}.</li>`).join(''):`<li>${t('Your tests will appear here.','你的測試會顯示在這裡。')}</li>`;
 ...
 $('massHelp').textContent=stage<2?t('Locked for this seal. Move the weight to explore. You can change the block count at the final seal.','這道封印的數量已固定。移動砝碼來探索；最後一道封印可以改變數量。'):t('Drag the weight up to add blocks, down to remove them. Or use + / −. Find two balanced designs.','向上拖動砝碼以增加數量，向下拖動以減少，或點 +／−。找出兩種平衡設計。');
...
function adjustMass(delta){
 ...
 $('mass').value=String(Math.max(1,Math.min(4,mass+delta)));changeSetting();testBeam();
}
...
function moveWeight(e){
 ...
 const dx=e.clientX-beamDrag.x,dy=e.clientY-beamDrag.y;
 if(!beamDrag.axis && Math.max(Math.abs(dx),Math.abs(dy))>8)beamDrag.axis=stage===2&&Math.abs(dy)>Math.abs(dx)?'mass':'distance';
 if(beamDrag.axis==='mass')$('mass').value=String(Math.max(1,Math.min(4,beamDrag.mass+Math.round(-dy/32))));
 else if(beamDrag.axis==='distance')$('distance').value=String(distanceFromX(e.clientX,rect.left,rect.width));
 changeSetting();
}
$('diagram').addEventListener('pointerdown',e=>{
 ...
 beamDrag={id:e.pointerId,mass,distance,result,feedback,x:e.clientX,y:e.clientY,axis:null};
...
$('diagram').addEventListener('pointerup',e=>{if(beamDrag&&beamDrag.id===e.pointerId){moveWeight(e);if(!beamDrag.axis){const r=$('diagram').getBoundingClientRect();$('distance').value=String(distanceFromX(e.clientX,r.left,r.width));changeSetting();}beamDrag=null;testBeam();}});
function cancelBeamDrag(){if(!beamDrag)return;({mass,distance,result,feedback}=beamDrag);beamDrag=null;render();}
