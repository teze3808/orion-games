...
function hints(){const c=chapters[chapter];return chapter===2?[
 t('Use the upper letter as the letter you want to send. The lower letter is its secret partner.','上方是你想寄出的字母，下方是它的秘密搭檔。'),
...
function resetMission(){chapter=0;offset=0;hintDepth=0;solved=false;messageState='';saveFailed=false;$('answer').value='';$('notes').open=false;render();}
$('answerForm').addEventListener('submit',e=>{e.preventDefault();submitAnswer($('answer').value);});
$('left').onclick=()=>{offset=(offset+25)%26;render();};$('right').onclick=()=>{offset=(offset+1)%26;render();};
$('hint').onclick=()=>{hintDepth=Math.min(3,hintDepth+1);render();};
$('next').onclick=()=>{if(!solved||chapter>=2)return;chapter++;offset=0;hintDepth=0;solved=false;messageState='';$('answer').value='';render();$('answer').focus();};
