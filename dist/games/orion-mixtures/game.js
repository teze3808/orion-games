...
function configureQuestions(v=OrionProgress.variant('mixtures')){questionVariant=v;
 const recipes=[
  [['iron','sand'],['sand','water'],['sand','salt','water']],
  [['iron','salt'],['sand','salt','water'],['iron','salt','water']],
  [['iron','sand','salt'],['iron','sand','water'],['iron','sand','salt','water']]
 ][v];
 recipes.forEach((mix,i)=>{
  const goal={magnet:mix.includes('iron')?['iron']:[],filter:i>0&&mix.includes('sand')?['sand']:[],working:i===0?mix.filter(x=>x!=='iron'):i===1?mix.filter(x=>!['iron','sand'].includes(x)):['salt'],vapor:i===2?['water']:[]};
  stages[i].mix=mix;stages[i].destinations=goal;
  const label={iron:['iron','鐵'],sand:['sand','沙'],salt:['salt','鹽'],water:['liquid water','液態水']};
  const description=lang=>Object.entries(goal).filter(([,a])=>a.length).map(([k,a])=>`${({magnet:['Magnet tray','磁鐵托盤'],filter:['Filter paper','濾紙'],working:['Sample jar','樣本瓶'],vapor:['Air','空氣']})[k][lang]}: ${k==='vapor'?(lang?'水蒸氣':'water vapour'):a.map(x=>label[x][lang]).join(' + ')}`).join(' · ');
  stages[i].goal=[`Recover these materials separately: ${description(0)}.`,`分開收集以下材料：${description(1)}。`];
 });
}
...
function renderWorkbench(){
 ...
 $('goals').innerHTML=checks.map((g,i)=>`<p class="goal ${inspected&&g.ok?'done':''}"><span aria-hidden="true">${inspected&&g.ok?'✓':'○'}</span> ${jarDrawing(g.materials,false,g.air)}${t(g.en,g.zh)} <small>${inspected&&g.ok?t('Collected','已收集'):t('Not yet','未完成')}</small></p>`).join('');
...
function check(){if(solved)return;const current={magnet,filter,working,vapor};const ok=inspected&&Object.entries(stages[stage].destinations).every(([key,materials])=>exactly(current[key],materials));if(!ok){say('wrong');return;}solved=true;if(stage===2)saveFailed=!OrionProgress.earn('crystal');say(stage===2?'complete':'solved');if(stage<2)$('next').focus();}
