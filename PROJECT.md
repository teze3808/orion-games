...
- Mission `seeds`, sigil `verdant` / Verdant / 青翠; route `games/orion-seeds/`.
- Biology objective: distinguish bean germination from healthy seedling growth and compare one environmental factor at a time.
- Three independent matched-pair trials within the mission: dry versus moist bean seeds in darkness; moist seedlings in dark versus light; moist bean seeds in light versus darkness. The final trial rejects equal lighting or unmatched moisture as evidence for a fair light test.
- Reward requires all three target conditions and a correct prediction for each. Incorrect predictions, dry or flooded conditions and unfair comparisons give pictured evidence and allow another fresh trial. Replay preserves sigils; reset-all synchronizes through the shared progress helper. Save failure offers a retry.
- Direct native moisture slider with +/− alternatives and a large light toggle. Full bilingual UI, live progress, left experiment/right result tablet layout, sticky evidence and illustrated plant states. Phones stack the panels. Optional hints start hidden. Biology assumptions and source documentation: `SCIENCE.md`; original built-in ImageGen prompts: `ARTWORK.md`.
- Tests: `node tests/seeds.cjs`. New release adds one mission, bringing the registry to 6 playable and 7 planned. Daily release date uses Asia/Hong_Kong.

Browser QA for the seed mission covered English landscape at 1024×768, Traditional Chinese portrait at 768×1024 and the stacked 390×844 phone layout. Verified pictured trial results, wrong predictions and unfair-test retries, progressive hints, award persistence, whole-card navigation, and confirmed/cancelled collection reset with language retained. Original artwork loaded and focal subjects remained visible. These are browser viewport checks, not a physical iPad Safari test. All six regression suites passed.
...
Validation: `node tests/random-challenges.cjs` exhausts all 30 variant sets in both languages, tests replay/reload/reset and 100 non-repeating full resets, plus blocked-save behavior. The browser verified a changed seed comparison after replay and reload. Native confirmation-dialog automation was unavailable during this run; confirmed/cancelled hub resets are covered by the existing automated hub tests.

## The Mist Keeper — 2026-09-26
Existing mission `mist` and sigil `mist` / Mist / 迷霧 are playable at `games/orion-mist/`. Seven playable, six planned (13 total). Find a boat in each of three ordered shores (8, 16, 32 caves) using truthful higher/lower comparisons. Each incorrect comparison discards impossible numbers. All three finds are required for the sigil; there is no attempt limit, timer or penalty. Binary search is discoverable through three hidden hints; optional final notes explain the midpoint strategy and worst-case comparison counts. The retry button clears the current search while retaining the target; full replay and hub reset choose a different one of eight persistent target sets. Every shore target changes with a changed variant. Shared bilingual and progress helpers preserve all existing sigils, language choice, storage-failure behavior and reset synchronization.
